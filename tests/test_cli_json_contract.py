...
def test_backfill_review_index_summaries_json_writes_single_json_document(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    summary_payload = {
    ...
    api = SimpleNamespace(
        backfill_review_index_summaries=lambda runs_root=None, run_id=None: SimpleNamespace(
            failed=[],
            to_dict=lambda: summary_payload,
        )
    )
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["alpaca-pipelines", "backfill-review-index-summaries", "--json"],
    )
